...
class HistoryB2BCouponList extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return HistoryB2BCouponListState();
  }
}

class HistoryB2BCouponListState extends State {
  ...
  loadData() async {
    ...
    await CouponController.to.getB2BHistoryData().then((value) {
      if (this.mounted) {
        if (value == null) {
          ...
          ISDatatable(
            ...
            rows: dataList.map((item) {
              return DataRow(cells: [
                ...
                DataCell(Center( child: SelectableText(Utils.getYearMonthFormat(Utils.getYearMonthDayFormat(item.INS_DATE.toString())) ?? '--', style: TextStyle(color: Colors.black), showCursor: true),)),
                ...
                //DataCell(Align(child: Text(item.SHOP_NAME.toString() ?? '--', style: TextStyle(color: Colors.black),),alignment: Alignment.centerLeft)),
                ...
                // ),
                // DataCell(Center(
                //     child: (item.ORDER_NO.toString() == null || item.ORDER_NO.toString() == 'null') ? Container() : IconButton(
                ...
                //           context: context,
                //           builder: (BuildContext context) => Dialog(
                //             child: OrderManager(orderNo: item.ORDER_NO.toString()),//CustomerOrder(custCode: custCode, custName: custName,),
                //           ),
                //         );
                //       },
                //     )),
                // ),
                //DataCell(Center(child: Text(item.SHOP_NAME.toString() ?? '--', style: TextStyle(color: Colors.black), ))),
              ]);
            }).toList(),
            columns: <DataColumn>[
