...
import 'package:daeguro_admin_app/View/TaxiManager/taxiCoupon_edit.dart';
import 'package:daeguro_admin_app/View/TaxiManager/taxiCoupon_regist.dart';
...
class TaxiCouponList extends StatefulWidget {
  @override
...
class CouponListState extends State {
  ...
  _regist() async {
    showDialog(
      ...
      builder: (BuildContext context) => Dialog(
        child: TaxiCouponRegist(couponTypeItems: couponTypeItems),
      ),
    ).then((v) async {
      if (v != null) {
        loadData();
  ...
  _edit() async {
    showDialog(
      context: context,
      builder: (BuildContext context) => Dialog(
        child: TaxiCouponEdit(couponTypeItems: couponTypeItems),
      ),
    ).then((v) async {
      if (v != null) {
        loadData();
          ...
          _couponType == 'IS_C400'
            ...
            rows: dataList.map((item) {
              return DataRow(cells: [
                ...
                // DataCell(Center(child: SelectableText('서비스' ?? '--', style: TextStyle(color: Colors.black), showCursor: true))),
