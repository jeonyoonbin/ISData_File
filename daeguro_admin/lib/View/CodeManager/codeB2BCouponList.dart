...
import 'package:daeguro_admin_app/View/CodeManager/codeB2BCouponEdit.dart';
import 'package:daeguro_admin_app/View/CodeManager/codeB2BCouponRegist.dart';
...
class CodeB2BCouponListManager extends StatefulWidget {
  @override
  CodeB2BCouponListManagerState createState() => CodeB2BCouponListManagerState();
}

class CodeB2BCouponListManagerState extends State<CodeB2BCouponListManager> {
  ...
  loadData() async {
    ...
    await CodeController.to.getListData('B2BCOUPON_TYPE', _useGbn).then((value) {
      if (value == null) {
        ...
        children: <Widget>[
          ...
          Row(
            children: [
              ...
              SizedBox(width: 8,),
              if (AuthUtil.isAuthCreateEnabled('44') == true)
              ISSearchButton(
                  label: '추가',
                  iconData: Icons.add,
                  onPressed: () => {
          ...
          ISDatatable(
            ...
            rows: dataList.map((item) {
              return DataRow(
                ...
                DataCell(Center(child: item.ETC_CODE2.toString() == 'Y' ? Icon(Icons.radio_button_unchecked, color: Colors.blue, size: 16) : Icon(Icons.clear, color: Colors.grey.shade400, size: 16))),
                    DataCell(Align(
                        child: Container(
                ...
                DataCell(Center(child: (item.ETC_CODE_GBN8.toString() == ' ' || item.ETC_CODE_GBN8.toString() == '' || item.ETC_CODE_GBN8 == null)
                        ...
                        child: InkWell(
                          ...
                          onTap: () {
                            showDialog(
                              context: context,
                              builder: (BuildContext context) => Dialog(
                                child: CodeB2BCouponEdit(sData: item),
                              ),
                            ).then((v) async {
                              if (v != null) {
                                await Future.delayed(Duration(milliseconds: 500), () {
