...
import 'package:daeguro_admin_app/Model/stat/statShopGunguModel.dart';
...
class StatShopGunguList extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return StatShopGunguListState();
  }
}

class DailyDataSource extends DataGridSource {
  DailyDataSource({List<StatShopGunguModel> dailyDataSource}) {
    _data = dailyDataSource
        .map<DataGridRow>((e) => DataGridRow(cells: [
      DataGridCell<String>(
          columnName: 'OPEN_DATE',
          value: e.OPEN_DATE.toString() == 'null'
              ? '합계'
              : DateFormat("yyyy-MM-dd").format(DateTime.parse(e.OPEN_DATE.toString())).toString()
      ),
...
class StatShopGunguListState extends State with AutomaticKeepAliveClientMixin{
  List<StatShopGunguModel> dataList = <StatShopGunguModel>[];
  ...
  loadData() async {
    ...
    await StatController.to.getShopGunguData(_divKey).then((value) {
      if (this.mounted) {
        if (value == null) {
          ...
          setState(() {
            value.forEach((element) {
              StatShopGunguModel temp = StatShopGunguModel.fromJson(element);
        ...
        children: <Widget>[
          ISSearchDropdown(
            ...
            item: [
              ...
              DropdownMenuItem(value: '1', child: Text('간편입점 사이트'),),
              DropdownMenuItem(value: '2', child: Text('관리앱'),),
