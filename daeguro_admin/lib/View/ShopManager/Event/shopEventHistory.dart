...
class ShopEventHistory extends StatefulWidget {
  final String shopCode;

  //final ShopBasicInfo sData;

  const ShopEventHistory({Key key, this.shopCode}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return ShopEventHistoryState();
  }
}

class ShopEventHistoryState extends State<ShopEventHistory> with SingleTickerProviderStateMixin {
  final GlobalKey<FormState> formKey = GlobalKey<FormState>();

  final List<ShopEventHistoryModel> dataHistoryList = <ShopEventHistoryModel>[];

  int current_tabIdx = 0;

  loadData() async {
    await ShopController.to.getEventHistoryData(widget.shopCode.toString(), '1', '10000').then((value) {
      if (value == null) {
        ISAlert(context, '정상조회가 되지 않았습니다. \n\n관리자에게 문의 바랍니다');
      } else {
        value.forEach((element) {
          ShopEventHistoryModel tempData = ShopEventHistoryModel.fromJson(element);
