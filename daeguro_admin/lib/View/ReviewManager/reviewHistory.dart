...
class ReviewHistory extends StatefulWidget {
  ...
  const ReviewHistory({Key key, this.seq}) : super(key: key);
...
class ReviewHistoryState extends State<ReviewHistory> with SingleTickerProviderStateMixin {
  final GlobalKey<FormState> formKey = GlobalKey<FormState>();

  final List<ShopReivewHistoryModel> dataHistoryList = <ShopReivewHistoryModel>[];

  int current_tabIdx = 0;

  loadData() async {
    await ReviewController.to.getHistoryData(widget.seq.toString(), '1', '10000').then((value) {
      if (value == null) {
        ISAlert(context, '정상조회가 되지 않았습니다. \n\n관리자에게 문의 바랍니다');
      }
      else {
        value.forEach((element) {
          ShopReivewHistoryModel tempData = ShopReivewHistoryModel.fromJson(element);
                ...
                children: [
                  ...
                  Container(
                      ...
                      child: SelectableText(
                        dataHistoryList[index].MEMO + ' [수정자 : (' + dataHistoryList[index].MOD_UCODE + ')' + dataHistoryList[index].USER_NAME + ']'  ?? '--',
