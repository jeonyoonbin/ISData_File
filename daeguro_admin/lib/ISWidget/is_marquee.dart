...
class ISMarquee extends StatefulWidget {
...
class _MarqueeState extends State<ISMarquee> with SingleTickerProviderStateMixin {
  double percent = 0.0;
  int current = 0;
  ...
  Widget build(BuildContext context) {
    ...
    if (textList.length == 1) {
      return Text(textList[0], style: widget.textStyle,);
    }

    if (textSpanList.length == 1) {
      return Center(
        child: Text.rich(textSpanList[0], style: widget.textStyle,),//TextStyle(fontSize: widget.fontSize, color: widget.textColor,),),
      );
    }
...
class _MarqueePainter extends CustomPainter {
  ...
  TextSpan getTextSpan(int position) {
    ...
    return TextSpan(text: text, style: textStyle);//TextStyle(fontSize: fontSize, color: textColor,),);
  }
  ...
  Offset _getTextOffset(
    ...
    var dx = 0.0;//size.width / 2 - width / 2;
