...
class ISListView<T extends TreeData> extends StatefulWidget {
  ...
  @override
  ISListViewState createState() => ISListViewState<T>();
}

class ISListViewState<T extends TreeData> extends State<ISListView<T>> {
  bool checkAll = false;
  ...
  Widget _getTableHeader() {
    var leading = widget.onSelected == null ? null :
          Checkbox(value: this.checkAll, onChanged: (v) {
            this._checkAll(v);
    ...
    if (widget.getRowOper != null) {
      list.insert(0, _getCell('operating', width: 100));
    }
    var result = Container(
  ...
  List<Widget> _getRowList(List<TreeVO<T>> data, TreeVO<T> parent) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      columnList = widget.columnData.map<Widget>((v) => _getCell(v.getData(vo.data), width: v.width),).toList();
      ...
      var leading = widget.onSelected == null ? null :
      Checkbox(value: vo.checked, onChanged: (v) {
          vo.checked = v;
      ...
      if (vo.children != null && vo.children.length > 0) {
        d = Container(decoration: _getBoxDecoration(), child: ExpansionTile(leading: leading, initiallyExpanded: true, children: _getRowList(vo.children, vo), title: title,
          ),
        );
      }
      else {
        d = Container(decoration: _getBoxDecoration(), child: ListTile(leading: leading, title: title,),
        );
      }
      list.add(d);
    }
